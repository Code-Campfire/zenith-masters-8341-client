...
export default function FriendCard({ children, friend, index }) {
	...
	let { first_name } = friend?.other_user ? friend.other_user : friend
	let { last_name } = friend?.other_user ? friend.other_user : friend

	first_name = first_name.length > 20 ? first_name.splice(0, 20) + '...' : first_name
	last_name = first_name.length > 20 ? first_name.splice(0, 20) + '...' : last_name
	...
	return (
		<div key={index} className="find-friends-card" onClick={() => console.log(friend, ' friend')}>
			...
			{<div>{friend?.other_user?.username}</div>}
			{children}
		</div>
	)
}
